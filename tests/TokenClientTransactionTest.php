...
class TokenClientTransactionTest extends TestCase {
  ...
  public function testCreateToken (): void {
    ...
    // Secret array
    $secret = [ 'fungible' => Crypto::generateSecret(), 'stackable' => Crypto::generateSecret(), 'env' => $env_secret, 'recipient' => [ Crypto::generateSecret(), Crypto::generateSecret(), Crypto::generateSecret(), Crypto::generateSecret(), Crypto::generateSecret(), ], 'receivers' => [ Crypto::generateSecret(), Crypto::generateSecret(), Crypto::generateSecret(), Crypto::generateSecret(), ], ];
    ...
    */
    ...
    $tokenMeta = [ 'name' => $this->token_slug[ 'fungible' ], 'fungibility' => 'fungible', 'splittable' => 0, 'supply' => 'limited', 'decimals' => 0, 'icon' => 'icon', ];
    $response = $this->client( $secret[ 'fungible' ] )
        ->createToken( $this->token_slug[ 'fungible' ], $full_amount, $tokenMeta );
    $this->checkResponse( $response );

    // --- Create a stackable token
    $tokenMeta = [ 'name' => $this->token_slug[ 'stackable' ], 'fungibility' => 'stackable', 'splittable' => 1, 'supply' => 'limited', 'decimals' => 0, 'icon' => 'icon', ];
    $response = $this->client( $secret[ 'stackable' ] )
        ->createToken( $this->token_slug[ 'stackable' ], $full_amount, $tokenMeta );
    $this->checkResponse( $response );

    // --- Create a ENV non-stackable & stackable tokens
    // ... non-stackable
    $tokenMeta = [ 'name' => $this->token_slug[ 'env.fungible' ], 'fungibility' => 'fungible', 'splittable' => 0, 'supply' => 'limited', 'decimals' => 0, 'icon' => 'icon', ];
    $response = $this->client( $secret[ 'env' ] )
        ->createToken( $this->token_slug[ 'env.fungible' ], $full_amount, $tokenMeta );
    $this->checkResponse( $response );
    // ... stackable token
    $tokenMeta = [ 'name' => $this->token_slug[ 'env.stackable' ], 'fungibility' => 'stackable', 'splittable' => 1, 'supply' => 'limited', 'decimals' => 0, 'icon' => 'icon', ];
    $response = $this->client( $secret[ 'env' ] )
        ->createToken( $this->token_slug[ 'env.stackable' ], $full_amount, $tokenMeta );
    $this->checkResponse( $response );

    // Save data
    $this->saveData( [ 'secret' => $secret, 'amount' => [ 'full' => $full_amount, 'transaction' => 100.000001, //1000.0 + 1.0/Decimal::$multiplier,
    ] ] );
  }
  ...
  public function testVIsotopeCombination (): void {
    ...
    // Recipients
    $recipients = [ array_get( $data, 'secret.fungible' ), array_get( $data, 'secret.recipient.0' ), array_get( $data, 'secret.recipient.1' ) ];
  ...
  protected function vIsotopeCombination ( $from_secret, $token, $recipients, bool $generate_wallets = false, int $transaction_amount = 1 ): Response {
    ...
    // Wallets
    $source_wallet = $client->queryBalance( $token, Crypto::generateBundleHash( $from_secret ) )
        ->payload();
    ...
    $molecule->addAtom( new \WishKnish\KnishIO\Client\Atom( $source_wallet->position, $source_wallet->address, 'V', $source_wallet->token, -$value, $source_wallet->batchId, null, null, null, null, $molecule->generateIndex() ) );

    // Add recipient wallets
    foreach ( $recipient_wallets as $recipient_wallet ) {

      // Initializing a new Atom to add tokens to recipient
      $molecule->addAtom( new \WishKnish\KnishIO\Client\Atom( $recipient_wallet->position, $recipient_wallet->address, 'V', $source_wallet->token, $value / count( $recipient_wallets ), $recipient_wallet->batchId, 'walletBundle', $recipient_wallet->bundle, null, null, $molecule->generateIndex() ) );

    }

    // Initializing a new Atom to deposit remainder in a new wallet
    $molecule->addAtom( new \WishKnish\KnishIO\Client\Atom( $remainder_wallet->position, $remainder_wallet->address, 'V', $source_wallet->token, $source_wallet->balance - $value, $remainder_wallet->batchId, 'walletBundle', $source_wallet->bundle, null, null, $molecule->generateIndex() ) );
  ...
  protected function claimShadowWallet ( string $token, string $recipient, array $intruders ): void {
    ...
    // Query
    $query = $this->client( $recipient )
        ->createQuery( QueryLinkIdentifierMutation::class );
    $response = $query->execute( [ 'bundle' => $bundleHash, 'type' => 'email', 'content' => $email, ] );
    if ( !$response->success() ) {
      $this->debug( $response, true );
    }
    ...
    $this->output( [ 'Identifier creating...', 'Bundle hash: ' . $bundleHash, 'Email: ' . $email, 'Verification code: ' . $code ] );

    // --- Try to create identifier with WRONG code: rejected
    $id_response = $this->client( $recipient )
        ->createIdentifier( 'email', $email, Strings::randomString( 8 ) );
    $this->assertEquals( $id_response->success(), false );

    // --- Bind a shadow wallet with RIGHT code
    $id_response = $this->client( $recipient )
        ->createIdentifier( 'email', $email, $code );
