#!/usr/bin/env bash
#
# Version consistency gate for the KnishIO PHP SDK.
#
...
# definition.
#
# PHP is the ONE repo where this gate cannot prevent a bad release. Packagist publishes from
# the git tag and crawls for new tags weekly even with webhooks disabled, so once a tag exists
# publication is inevitable and permanent. Prevention lives in scripts/release-sdk.sh in the
# monorepo, which runs this script BEFORE creating a tag. This copy is the second opinion.
#
...
# composer.json's `version`. Packagist itself derives the published version from the git TAG
# and recommends omitting this field — but it is load-bearing here: self-test.php reports it
# into php-results.json (which the cross-SDK gauntlet's coherence gate reads) and the
# `changelog` job in php.yml diffs it. Since it cannot simply be dropped, it must be gated.
collect "composer.json version" \
  "$(sed -n 's/.*"version"[[:space:]]*:[[:space:]]*"\([^"]*\)".*/\1/p' composer.json | head -1)"
# -----------------------------------------------------------------------------------------
