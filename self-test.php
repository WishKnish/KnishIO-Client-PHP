...
function test_mlkem768() {
    ...
    try {
        ...
        $publicKey = $encryptionWallet->pubkey;
        $rawLen = strlen(base64_decode($publicKey));
        $publicKeyGenerated = !empty($publicKey) && $rawLen === 1184;
        log_test('Encryption wallet creation', true);
        log_test('ML-KEM768 determinism check', $keysIdentical);
        log_test('ML-KEM768 public key generation', $publicKeyGenerated);
        
        // Real encryption test using ML-KEM encapsulation + AES-GCM (matches JavaScript)
        $encryptedData = $encryptionWallet->encryptMessageML($testConfig['plaintext'], $publicKey);
        $cipherLen = strlen(base64_decode($encryptedData['cipherText']));
        $encryptionSuccess = !empty($encryptedData['cipherText']) && 
                           !empty($encryptedData['encryptedMessage']) &&
                           $encryptedData['cipherText'] !== $encryptedData['encryptedMessage'] &&
                           $cipherLen === 1088;
        
        log_test('Message encryption (self-encryption)', $encryptionSuccess);
        
        // Real decryption test using ML-KEM decapsulation + AES-GCM
        $decryptedMessage = $encryptionWallet->decryptMessageML($encryptedData);
