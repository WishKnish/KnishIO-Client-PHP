...
The GraphQL transport (Guzzle) issues a fresh network request per query — there
...
Client integration: `KnishIOClient::setSecretStorage(SecretStorageProvider $storage, ?string $bundleHash)` attaches a provider and persists the secret there; the client still holds the cleartext in memory.
